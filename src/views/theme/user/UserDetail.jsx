...
const isAbortError = (err) =>
  err?.name === 'AbortError' || err?.code === 20 || /aborted|abort/i.test(err?.message || '')
      ...
      <CCard className="bw-card">
        <CCardHeader className="bw-header d-flex align-items-center justify-content-between">
          ...
          <div className="d-flex align-items-center" style={{ gap: 8 }}>
            <button className="bw-btn-ghost" onClick={() => navigate(-1)}>
              Quay lại
            </button>
              ...
              <CRow className="mb-3">
                <CCol md={6}>
                  <CListGroup flush>
                    ...
                    <Item label="Email">
                      {seller?.email ? seller.email : <span className="text-muted">-</span>}
                    </Item>
                    ...
                    {(data?.addresses?.length ? data.addresses : []).map((a, i) => (
                      <div key={i} className="addr-item">
                        ...
                        <div>
                          {a.address}{' '}
                          {a.is_default ? (
                            <CBadge className="bw-badge-available ms-2">mặc định</CBadge>
                          ) : null}
                        </div>
                      </div>
                    ))}
              ...
              <CCol md={6}>
                <CListGroup flush>
                  ...
                  <Item label="Ảnh gian hàng">
                    {seller.avatar_link ? (
                      <img src={seller.avatar_link} alt="shop-avatar" className="bw-avatar" />
                    ) : (
                      '-'
                    )}
                  </Item>
